...
$$ \text{To an attention model, which is a kind of neural network, we pass in} \ h_t \ \text{(decoder's hidden state) and} \ f_j \ \text{(feature vector coming from the encoder).\break
