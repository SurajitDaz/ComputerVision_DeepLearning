...
To an attention model, which is a kind of neural network, we pass in $ \ h_t \ $ (decoder's hidden state) and $ \ f_j \ $ (feature vector coming from the encoder). Therefore with this information, we can get how important is the $ \ j_{th} \ $ pixel of the input image at the $ \ t_{th} \ $ time stamp.
